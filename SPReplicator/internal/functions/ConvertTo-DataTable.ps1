...
function ConvertTo-DataTable {
    ...
    begin {
        Write-PSFMessage -Level Debug -Message "Bound parameters: $($PSBoundParameters.Keys -join ", ")"
        Write-PSFMessage -Level Debug -Message "TimeSpanType = $TimeSpanType | SizeType = $SizeType"
    ...
    process {
                        ...
                        try {
                            ...
                            if ($property.Name -notin $columns) {
                                try {
                                    ...
                                    Write-PSFMessage -Level Warning -Message "Failed to add property $($property.Name) from $object" -ErrorRecord $_ -Target $object
                                }
                            }
                            else {
                                Write-PSFMessage -Level Warning -Message "Failed to add property $($property.Name) from $object" -ErrorRecord $_ -Target $object
        ...
        Write-PSFMessage -Level InternalComment -Message "Finished."
         , $datatable
    }
}