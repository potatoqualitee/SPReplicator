...
Describe "OnPrem Integration Tests" -Tag "IntegrationTests" {
    BeforeAll {
    ...
    Context "New-SPRList" {
        ...
        It "Creates a new list named $script:mylist" {
            $results = New-SPRList -Site $script:site -Title $script:mylist -Description "My List Description"
    ...
    Context "Add-SPRColumn" {
        ...
        It "Adds a column named TestColumn" {
            $results = Add-SPRColumn -Site $script:site -List $script:mylist -ColumnName TestColumn -Description "One column"
    ...
    Context "Add-SPRListItem" {
        ...
        It "Autocreates new list and adds new items as user System Account" {
            ...
            $object += [pscustomobject]@{ Title = 'Hello2'; TestColumn = 'Sample Data2'; }
            $object += [pscustomobject]@{ Title = 'Hello3'; TestColumn = 'Sample Data3'; }
            $results = $object | Add-SPRListItem -Site $script:site -List $newList -AutoCreateList -AsUser 'System Account'
            $results.Title | Should -Be 'Hello', 'Hello2', 'Hello3'
            $results.TestColumn | Should -Be 'Sample Data', 'Sample Data2', 'Sample Data3'
    ...
    Context "Update-SPRListItemAuthorEditor" {
        It "Supports WhatIf" {
            InModuleScope SPReplicator { Mock Test-PSFShouldProcess { $null } }
            $results = Get-SPRListItem -Site $script:site -List $script:mylist | Select-Object -First 1 | Update-SPRListItemAuthorEditor -Identity 'System Account' -Confirm:$false -WhatIf
            $results | Should -Be $null
            Import-Module SPReplicator -Force
        }
        It "Updates author/editor for a single item on $script:mylist" {
            $results = Get-SPRListItem -Site $script:site -List $script:mylist | Select-Object -First 1 | Update-SPRListItemAuthorEditor -Identity 'System Account' -Confirm:$false
    ...
    Context "New-SPRLogList" {
        ...
        It "Creates a new log list" {
            $results = New-SprLogList -Site $script:site -Title SPReplicator
    ...
    Context "Remove-SPRListItem" {
        ...
        It "Removes specific data from $script:mylist" {
            $row = Get-SPRListItem -Site $script:site -List $script:mylist -Id $script:id
...
Describe "OnPrem Final Tests" -Tag "Finaltests" {
